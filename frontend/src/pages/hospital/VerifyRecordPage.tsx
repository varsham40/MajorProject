...
export const VerifyRecordPage: React.FC = () => {
  ...
  return (
    <div className="p-6 space-y-6 max-w-4xl mx-auto">
      <div>
        <h1 className="text-2xl font-extrabold text-white">Record Tamper Integrity Verification Engine</h1>
        <p className="text-xs text-slate-400">Recalculates canonical SHA-256 fingerprint from PostgreSQL and validates against Hardhat blockchain ledger</p>
      </div>
      ...
      <div className="glass-panel p-8 rounded-2xl border border-slate-800 space-y-6">
        ...
        <form onSubmit={handleVerify} className="space-y-4">
          <div className="space-y-1">
            <label className="text-xs font-semibold text-slate-300">Select Received Record to Verify</label>
            <select
        ...
        {result && (
          <div className={`p-6 rounded-2xl border space-y-4 ${
            ...
            {result.current_hash && (
              <div className="space-y-1 text-xs font-mono pt-3 border-t border-slate-800 text-slate-300">
                <p>Current Off-Chain Hash: <span className="text-cyan-400">{result.current_hash}</span></p>
