...
export const HospitalDashboard: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-4 text-cyan-400">
  ...
  const diseaseColors: Record<string, string> = {
    ...
    'Diabetes': 'from-cyan-500 to-blue-600 text-cyan-400 bg-cyan-500',
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-8 max-w-7xl mx-auto">
      {/* Hero Header Banner */}
      <div className="relative overflow-hidden bg-slate-900 border border-slate-800 rounded-3xl p-6 lg:p-8 shadow-2xl backdrop-blur-xl">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1 flex flex-wrap items-center gap-x-4 gap-y-1">
                  <span>License: <strong className="text-slate-200 font-mono">{data?.license_number || 'LIC-VERIFIED'}</strong></span>
                  <span>Facility: <strong className="text-slate-200">{data?.address || 'Main Campus'}</strong></span>
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-emerald-500 hover:from-cyan-400 hover:to-emerald-400 text-slate-950 font-black text-xs shadow-lg shadow-cyan-500/20 transition flex items-center gap-2 cursor-pointer active:scale-95"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-3 hover:border-cyan-500/30 transition shadow-lg">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Affiliated Doctors</span>
            <div className="p-2 bg-cyan-500/10 text-cyan-400 rounded-xl border border-cyan-500/20">
              <Stethoscope className="w-4 h-4" />
            </div>
          </div>
          <div>
            <p className="text-3xl font-black text-white">{kpis.total_doctors || 0}</p>
            <p className="text-[11px] text-slate-400 mt-1 font-medium">Physician Staff Active</p>
          </div>
        </div>

        {/* Card 2: Facility Diagnostic Records */}
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-3 hover:border-emerald-500/30 transition shadow-lg">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Hospital Records</span>
            <div className="p-2 bg-emerald-500/10 text-emerald-400 rounded-xl border border-emerald-500/20">
              <FolderOpen className="w-4 h-4" />
            </div>
          </div>
          <div>
            <p className="text-3xl font-black text-white">{kpis.total_records || 0}</p>
            <p className="text-[11px] text-slate-400 mt-1 font-medium">Generated at Facility</p>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-3 hover:border-cyan-500/30 transition shadow-lg">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Consented Access</span>
            <div className="p-2 bg-cyan-500/10 text-cyan-400 rounded-xl border border-cyan-500/20">
              <Key className="w-4 h-4" />
            </div>
          </div>
          <div>
            <p className="text-3xl font-black text-white">{kpis.authorized_records || 0}</p>
            <p className="text-[11px] text-slate-400 mt-1 font-medium">Patient Access Tokens</p>
          </div>
        </div>

        {/* Card 4: Outward Patient Grants */}
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-3 hover:border-purple-500/30 transition shadow-lg">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Outward Grants</span>
            <div className="p-2 bg-purple-500/10 text-purple-400 rounded-xl border border-purple-500/20">
              <ExternalLink className="w-4 h-4" />
            </div>
          </div>
          <div>
            <p className="text-3xl font-black text-white">{kpis.shared_records || 0}</p>
            <p className="text-[11px] text-slate-400 mt-1 font-medium">Patient Shared External</p>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-3 hover:border-emerald-500/30 transition shadow-lg">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Blockchain Audit</span>
            <div className="p-2 bg-emerald-500/10 text-emerald-400 rounded-xl border border-emerald-500/20">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1 font-medium">SHA-256 Smart Contract</p>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-5 shadow-xl">
          <div className="flex justify-between items-center border-b border-slate-800 pb-4">
            <div>
              ...
              <p className="text-xs text-slate-400">Breakdown of AI predictions generated across disease categories at your facility.</p>
            </div>
          ...
          {Object.keys(diseaseDist).length === 0 ? (
            ...
            <div className="space-y-4">
              {Object.entries(diseaseDist).map(([disease, count]) => {
                ...
                const colorConfig = diseaseColors[disease] || 'from-cyan-500 to-blue-600 text-cyan-400 bg-cyan-500';

                return (
                  <div key={disease} className="space-y-1.5 bg-slate-950/60 p-3.5 rounded-2xl border border-slate-800/80">
                    <div className="flex justify-between items-center text-xs font-bold">
                      <span className="text-slate-200">{disease}</span>
                      <span className="text-slate-400 font-mono">
                        <strong className="text-white">{numCount}</strong> records ({percent}%)
                      </span>
                    </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-4 shadow-xl">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="bg-slate-950/60 p-3 rounded-xl border border-slate-800 space-y-1">
                <span className="text-slate-400 font-medium">Scheduled (Pending)</span>
                <p className="text-xl font-bold text-amber-400">{aptMetrics.scheduled || 0}</p>
              </div>

              <div className="bg-slate-950/60 p-3 rounded-xl border border-slate-800 space-y-1">
                <span className="text-slate-400 font-medium">Accepted</span>
                <p className="text-xl font-bold text-cyan-400">{aptMetrics.accepted || 0}</p>
              </div>

              <div className="bg-slate-950/60 p-3 rounded-xl border border-slate-800 space-y-1">
                <span className="text-slate-400 font-medium">Completed (Treated)</span>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-3 shadow-xl">
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono text-white font-bold">{riskDist.high_risk || 0}</span>
              </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono text-white font-bold">{riskDist.medium_risk || 0}</span>
              </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono text-white font-bold">{riskDist.low_risk || 0}</span>
              </div>
            </div>
          </div>
        </div>
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {topDoctors.map((doc: any) => (
                    <tr key={doc.id} className="hover:bg-slate-800/40 transition">
                      <td className="py-3 px-4 font-bold text-white">{doc.name}</td>
                      <td className="py-3 px-4 text-cyan-400 font-medium">{doc.specialization}</td>
          ...
          {recentActivities.length === 0 ? (
            ...
            <div className="space-y-3">
              {recentActivities.map((act: any) => (
                <div key={act.id} className="p-3 bg-slate-950/80 rounded-2xl border border-slate-800 space-y-1">
                  <div className="flex justify-between items-center text-xs">
                    <span className="font-bold text-cyan-400 font-mono">{act.title}</span>
                    <span className="text-[10px] text-slate-500">{act.timestamp}</span>
                  </div>
                  <p className="text-xs text-slate-300">
                    Analysis: <strong className="text-white">{act.disease}</strong> ({act.result})
                  </p>
                </div>
              ))}
            </div>
