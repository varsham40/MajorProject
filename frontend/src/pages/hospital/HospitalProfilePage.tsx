...
export const HospitalProfilePage: React.FC = () => {
  ...
  if (loading) {
    return <div className="p-8 text-emerald-600 text-center font-semibold animate-pulse">Loading Hospital Profile...</div>;
  }

  return (
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      <div>
        <h1 className="text-2xl font-extrabold text-slate-900 font-bold">Healthcare Organization Profile</h1>
        <p className="text-xs text-slate-500">Institutional credentials, facility contact details, and node security</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left Column: Hospital Identity Card */}
        <div className="glass-panel p-6 rounded-2xl space-y-5 text-center flex flex-col items-center justify-between border border-slate-200/80">
          <div className="space-y-4 w-full">
            ...
            <div>
              <h2 className="text-xl font-extrabold text-slate-900 font-bold">{profile?.name || 'Main Hospital'}</h2>
              <p className="text-xs text-purple-400 font-bold mt-0.5">Verified Medical Organization</p>
              <span className="inline-block mt-2 text-[10px] font-mono font-bold px-3 py-1 rounded-full bg-slate-100 text-slate-600">
            ...
            <div className="w-full text-xs text-left space-y-2 pt-4 border-t border-slate-200/80 text-slate-600">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-slate-900 font-bold">{profile?.email || user?.email}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Facility Phone:</span>
                <span className="font-semibold text-slate-900 font-bold">{profile?.phone || 'N/A'}</span>
              </div>
              <div className="flex justify-between">
          ...
          <div className="bg-white border border-slate-200/80 rounded-2xl p-6 space-y-4 shadow-xl">
            ...
            <form onSubmit={handleSaveProfile} className="space-y-3 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#f4f7f6] border border-slate-200/80 rounded-xl px-3 py-2 text-slate-900 font-bold focus:border-purple-500"
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-slate-600 font-semibold mb-1">License / Accreditation</label>
                  <input
                    type="text"
                    value={licenseNumber}
              ...
              <div>
                <label className="block text-slate-600 font-semibold mb-1">Facility Street Address</label>
                <textarea
                  rows={2}
                  value={address}
                  onChange={(e) => setAddress(e.target.value)}
                  placeholder="Complete hospital physical location address..."
                  className="w-full bg-[#f4f7f6] border border-slate-200/80 rounded-xl p-3 text-slate-900 font-bold focus:border-purple-500"
                />
              </div>
          ...
          <div className="bg-white border border-slate-200/80 rounded-2xl p-6 space-y-4 shadow-xl">
            ...
            <form onSubmit={handleChangePassword} className="space-y-3 text-xs">
              ...
              <button
                type="submit"
                disabled={changingPw}
                className="px-5 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-700 text-slate-800 font-bold text-xs border border-slate-200 transition-all flex items-center gap-2 cursor-pointer"
              >
