...
export const DoctorProfilePage: React.FC = () => {
  ...
  if (loading) {
    return <div className="p-8 text-cyan-400 text-center font-semibold">Loading doctor profile...</div>;
  }

  return (
    <div className="p-6 space-y-6 max-w-4xl mx-auto">
      <div>
        <h1 className="text-2xl font-extrabold text-white">Doctor Professional Profile</h1>
        <p className="text-xs text-slate-400">Practitioner credentials, specialization details, and account security</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass-panel p-6 rounded-2xl space-y-5 text-center flex flex-col items-center justify-between">
          <div className="space-y-4 w-full">
            <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-cyan-500 to-emerald-400 mx-auto flex items-center justify-center text-dark-900 font-extrabold text-4xl shadow-xl shadow-cyan-500/20">
              <Stethoscope className="w-12 h-12" />
            </div>

            <div>
              <h2 className="text-lg font-extrabold text-white">{profile?.name || 'Doctor'}</h2>
              <p className="text-xs text-cyan-400 font-bold mt-0.5">{profile?.specialization || 'General Physician'}</p>
              <span className="inline-block mt-2 text-[10px] font-mono px-3 py-1 rounded-full bg-slate-800 text-slate-300">
            ...
            <div className="w-full text-xs text-left space-y-2 pt-4 border-t border-slate-800 text-slate-300">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-white">{profile?.department || 'General Medicine'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">License No:</span>
                <span className="font-mono text-cyan-400 font-bold">{profile?.license_number || 'N/A'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Affiliated Hospital:</span>
                <span className="font-semibold text-white">{profile?.hospital_name}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 space-y-4 shadow-xl">
            ...
            <form onSubmit={handleProfileSave} className="space-y-3 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={profile?.email || ''}
                    disabled
                    className="w-full bg-slate-950/60 border border-slate-800/80 rounded-xl px-3 py-2 text-slate-400 cursor-not-allowed font-mono"
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                disabled={saving}
                className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-emerald-500 text-dark-900 font-extrabold text-xs shadow-lg shadow-cyan-500/20 disabled:opacity-50 transition-all hover:opacity-90 flex items-center gap-2"
              >
