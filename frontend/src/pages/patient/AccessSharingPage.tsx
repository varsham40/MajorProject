...
export const AccessSharingPage: React.FC = () => {
  ...
  if (loading) return <div className="p-8 text-cyan-400 text-center font-semibold">Loading Access & Consent Sharing...</div>;

  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div>
        <h1 className="text-2xl font-extrabold text-white">Patient Record Sovereignty & Access Sharing</h1>
        <p className="text-xs text-slate-400">Generate time-limited, report-specific access tokens to grant doctors access to your diagnostic reports</p>
      </div>
        ...
        <div className="glass-panel p-6 rounded-2xl space-y-4 border border-cyan-500/30">
          ...
          <form onSubmit={handleGenerateToken} className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-300">Select Specific Medical Analysis Report</label>
              <select
                value={selectedRecord}
                onChange={(e) => setSelectedRecord(e.target.value)}
                className="w-full bg-dark-800 border border-slate-700 rounded-xl px-3.5 py-2.5 text-xs text-white focus:outline-none focus:border-cyan-500"
              >
            ...
            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-300">Authorized Healthcare Organization (Hospital)</label>
              <select
                required
            ...
            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-300">Authorized Attending Physician (Doctor)</label>
              <select
                required
            ...
            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-300">Token Duration (Hours)</label>
              <select
                value={expiryHours}
          ...
          {generatedToken && (
            <div className="p-4 rounded-xl bg-cyan-500/10 border border-cyan-500/40 space-y-2 text-center">
              <span className="text-xs text-slate-400 font-semibold">Authorized Access Token Code</span>
              <div className="flex items-center justify-center gap-2">
                <span className="text-xl font-mono font-extrabold text-cyan-400 tracking-wider">{generatedToken}</span>
                <button
                  onClick={() => copyToClipboard(generatedToken)}
                  className="p-1.5 rounded-lg bg-dark-800 hover:bg-slate-700 text-slate-300"
                >
                  {copied ? <Check className="w-4 h-4 text-emerald-400" /> : <Copy className="w-4 h-4" />}
                </button>
              </div>
              <p className="text-[10px] text-slate-400">Share this token code with your doctor to authorize viewing of your report.</p>
            </div>
          )}
        </div>

        {/* Active Tokens Table */}
        <div className="glass-panel p-6 rounded-2xl space-y-4 lg:col-span-2 border border-slate-800">
          <h2 className="text-base font-bold text-white flex items-center gap-2">
                  ...
                  <tr key={t.id} className="hover:bg-slate-800/30">
                    <td className="py-3 px-3">
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-extrabold text-cyan-400 text-xs tracking-wider">{t.token_code}</span>
                        <button
                          onClick={() => copyRowTokenToClipboard(t.token_code)}
                          title="Copy token code"
                          className="p-1 rounded-lg bg-dark-800 hover:bg-slate-700 text-slate-400 hover:text-cyan-300 transition-all border border-slate-700/60"
                        >
                    ...
                    <td className="py-3 px-3 text-slate-200">
                      <span className="font-semibold text-white">{t.doctor_name || 'Dr. Rahul Sharma'}</span>
                      <br/>
                      <span className="text-[10px] text-slate-400">{t.hospital_name || 'Metro General Hospital'}</span>
                    </td>
                    <td className="py-3 px-3 text-slate-400 text-[11px]">{formatTimestamp(t.expires_at)}</td>
                    <td className="py-3 px-3">
                      <span className={`px-2.5 py-0.5 rounded-full font-bold uppercase text-[10px] ${
                        t.status === 'ACTIVE' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-slate-800 text-slate-500'
                      }`}>
                        {t.status}
                      </span>
                    </td>
                    <td className="py-3 px-3 text-right">
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg bg-rose-500/10 hover:bg-rose-500 text-rose-400 hover:text-white border border-rose-500/30 text-[11px] font-bold transition-all flex items-center gap-1.5 ml-auto"
                      >
